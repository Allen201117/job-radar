...
# ⚠️ **顺序就是优先级，改顺序前先读这段**。两条铁律：
#   ① **带「报名」前缀的一律排在裸「截止」前面**；② 裸「截止」必须挡掉「XX计算截止时间」。
# 2026-09-18 线上走查实测：上海交通职业技术学院那条正文里有三句带「截止」——
#   「**报名**截止时间:即日起至9月25日止」（真的）、「**居住证有效期计算**截止时间为2026年12月31日」、
#   「**工作经历计算**截止时间为2026年9月30日」。裸「截止」那条当时排在前面，于是抽走了 12-31，
#   一条 9/25 就截止的公告被挂到年底。「年龄/学历/工作经历计算截止时间」是事业单位公告的标配写法，
#   不是个案。
# 「截止」前面这么多字内出现下列词，说明它算的是**资格条件的时点**（年龄/学历/工作经历怎么算到哪天），
# 不是报名窗口。Python 的逆序断言要求定宽、装不下多字词的或，所以改成匹配完再回看前文。
_QUALIFY_WORDS = re.compile(r"(计算|有效期|年龄|学历|学位|工作经历|工作年限|服务期|户籍|缴费|资格审查)")
_QUALIFY_LOOKBACK = 14

_RANGE_END_PATS = [
    # ── 第一梯队：带「报名」前缀，指向明确 ────────────────────────────────
    # 「报名时间：2026年9月10日…至2026年9月20日…」
    re.compile(r"报名[时實]间" + _GAP + r"{0,50}?至" + _GAP + r"{0,25}?" + _DATE),
    # 「报名截止时间：2026年9月20日」
    re.compile(r"报名" + _GAP + r"{0,40}?截止" + _GAP + r"{0,25}?" + _DATE),
    # 「报名时间：9月10日…至9月20日…」（年份省略）
    re.compile(r"报名[时實]间" + _GAP + r"{0,50}?至" + _GAP + r"{0,25}?" + _MD),
    # 「报名截止时间：即日起至9月25日止」（年份省略）—— 必须排在裸「截止」**前面**，见上文。
    re.compile(r"报名" + _GAP + r"{0,40}?截止" + _GAP + r"{0,25}?" + _MD),
    # 「报名时间：2026年9月18日9:00-2026年9月24日16:00」：区间，取右端。
    ...
    # 「报名方式（一）自公告发布之日起至2026年9月22日」：只有一个「至」。
    re.compile(_REG + _GAP + r"{0,25}?至" + _GAP + r"{0,12}?" + _DATE),
    # 「自公告发布之日（起）至 X」：它常与「报名」**跨句**（「1.网上报名。自公告发布之日至9月30日。」），
    # 而 _GAP 刻意不跨句 —— 这个短语指向性已足够强，单独收。
    re.compile(r"自" + _GAP + r"{0,12}?之日(?:起)?至" + _GAP + r"{0,12}?" + _DATE),
    re.compile(r"自" + _GAP + r"{0,12}?之日(?:起)?至" + _GAP + r"{0,12}?" + _MD),
    # ── 第二梯队：不带「报名」的裸「截止」，必须挡掉资格条件的计算时点 ──────────
    re.compile(r"截止(?:时间|日期)?\s*[为：:]{0,3}\s*" + _DATE),
    re.compile(_DATE + r"\s*截止"),
    re.compile(r"截止(?:时间|日期)?\s*(?:为|：|:)?\s*" + _MD),
]
# 上面最后三条是「裸截止」——它们要过 _QUALIFY_WORDS 回看门（见 extract_deadline）。
_BARE_DEADLINE_FROM = len(_RANGE_END_PATS) - 3
...
def extract_deadline(
    ...
    for idx, pat in enumerate(_RANGE_END_PATS):
        m = None
        for cand in pat.finditer(norm):
            # 裸「截止」：回看前 14 字，命中资格条件词就跳过这一处，继续找下一处。
            if idx >= _BARE_DEADLINE_FROM and _QUALIFY_WORDS.search(
                    norm[max(0, cand.start() - _QUALIFY_LOOKBACK):cand.start()]):
                continue
            m = cand
            break
        if not m:
