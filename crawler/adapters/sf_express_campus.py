...
## 为什么没上 RepetitionBrake
它是给「同一个岗 × N 家门店」的批量发布源用的（星巴克/来伊份/喜茶归一标题重复率 96~99.6%）。
2026-09-18 用同一套判据量过这个源：120 个岗 → 92 种归一标题，**重复率 23.3%**，
比「正常源」那一档（奇瑞 47.7% / 新东方 46.4%）还低一半，装了也永远刹不住，只是多一份状态。
真哪天顺丰在这个门户上批量发门店岗，再按这个数字重新评估。
