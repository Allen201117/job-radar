...
岗位已撤时页面渲染「职位不存在」之类，状态码查不出来（按状态码抽样的 probe-dead-links.js 只查得出硬 404=0.7%，
该脚本 2026-09-24 已删）。
...
_BROWSER_ADAPTERS = (
    ...
    "alibaba", "netease", "ctrip",
    # huawei 已移出（2026-07-29）：其详情走公开 JSON 接口 getJobDetail/newHr，httpx 直接判死+补正文
    # → 已进 ENRICH_REGISTRY/liveness-sweep。留在这里只会无头重复探活，且渲染出来的页面
    # 命不中 DEAD_MARKERS（实测探 300+ 岗下架 0 个）＝纯浪费（同 google/wt/hotjob 的处理）。
...
DEAD_MARKERS_STRONG = [
    ...
    # 老版北森门户（/zpdetail/{数字}，25 个租户同一套模板）撤岗后仍渲染岗位名，只多一句
    # 「对不起，此职位已停用。」——标题在场，不收这句就被判 alive（2026-09-24 全集 2,306 岗逐条取页：
    # 85 岗是这个页面且都没有正文段；1,926 个有正文的在招页原始 HTML 里 0 处「已停用」；
    # 全库 46.6 万 active 的标题 / 正文 0 处出现这句）。收整句、不收「已停用」裸词。
    "此职位已停用",
]
