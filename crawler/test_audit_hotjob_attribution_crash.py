...
独立于 test_audit_hotjob_attribution.py（那个文件测的是纯判据 cjk_name/names_agree/audit），
避免混在一起改坏了不好定位。
"""
import sys
import unittest
...
import audit_hotjob_attribution as M
...
class MainCrashRecordsFailedLedgerTest(unittest.TestCase):
    def test_crash_after_supabase_obtained_writes_failed_and_reraises(self):
        with patch("db.get_supabase", return_value="fake-supabase-client"), \
    ...
    def test_supabase_itself_failing_reraises_without_calling_record_ops_run(self):
        with patch("db.get_supabase", side_effect=ConnectionError("no db")), \
             patch.object(M.ops_runs, "record_ops_run") as rec:
