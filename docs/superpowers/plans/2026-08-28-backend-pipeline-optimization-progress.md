...
- [x] 修复后重跑：**真开出 10 个 Issue**（#5-#14），告警出口首次验证走通。逐条分诊：
      · #10 LLM 欠费 = 充值前旧事件（48h 窗内），会自然过期
      · #12 美团分片跑满 330min 被杀（08-23）= 本轮 20h 冷却正是治它；最新一轮该分片 checked=0（冷却跳过），已消解
      · #13/#14 db-report / production-smoke「47 天没跑」= cron 昨晚 21:11 才加、今日 03:30 才首次有机会触发，**早报半小时不算误报**，明日巡检自动复核；已手动跑 db-report 验证 workflow 本身 success
      · #5/#6 auto_discover 零产出 = 候选池真枯竭（审阅 C4 已定性，诚实的 0）
      · #7/#8 数据均在 08-26/27，早于今日推的搜索额度预留修复，待明日复核
      · #9 台账卡单 = 回写能力上线前的历史残留 → **已用项目自带 CLI 结清 8 条，queued 归零**
- [x] **同日效果验证（vs 今晨基线，仅半天）**：
      | 指标 | 今晨基线 | 收尾复测 | 变化 |
      |---|---|---|---|
      | 从未探活 | 77,065 | **65,492** | −11,573（−15%） |
      | 7 天未探 | 198,154 | **152,934** | −45,220（−23%） |
      | 待清死岗 expired | 440 | **5,993** | 盲区探活器挖出 ~5.5k 真死岗 |
      | feishu 死门户坏链 | 1,006 active | 0 | 已 removed |
      结论：预算重分配与盲区清零**当天即生效**，两个欠账指标同步下降（不是「探得更多」而是「探得更准」——
      active 总数仅 381,529→380,403，说明减少的是重复劳动而非漏探）。
