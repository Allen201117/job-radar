...
## 7. 爬虫性能实测（7 天 crawl_runs，29,325 次运行 / 1,466 源，2026-09-03 补）

统计脚本按 adapter 聚合 p50/p95/总耗时/产出/失败率/空产出率，再下钻到单源。全部 7 天抓取 runner 时间 176 小时。

| 病灶 | 数据 | 根因 | 处置 |
|---|---|---|---|
| 国聘一周 16.9h，平均每轮 9 岗 | 28 源，慢轮 450~570s，found=2 而列表 738 条 | `_enrich_details` 对前 300 条**串行**取详情核验，且在按 `match` 核名过滤**之前**，配额全花在无关公司上 | ✅ 先过滤再核验 + 3 并发（4a0db50）；live 对拍中国石油 jobs 2→251 |
| 11 个源 28 轮全失败 | Workday×4 / phenom×2 / ashby / eightfold / wt / alibaba / beisen | 站点名错（Otis/Takeda 全 wd 域 422）、板块改名（Fireworks→fireworks）、反爬 403（AstraZeneca）、平台搬家（固德威→大易；海信北森租户已 404）、CI 出口 IP 被拦（Thermo/Mondelez/PepsiCo/AMD 本机 200、CI 4xx/5xx）、海信穿透到浏览器分支崩溃 | ✅ 改 URL 1 / 停用 5 / beisen 防穿透（含单测）；IP 被拦的 4 家保留，见 §9 自托管 runner |
| 模块级绿灯看不见源级死亡 | 上面 11 个源一周无人知晓 | crawl_runs 每轮一行 failed，ops_runs 只看模块 | ✅ 看门狗规则 F「源连续失败」（1b31a9b） |
| Workday 一周 46.9h（最大黑洞） | 100 源，p95 216s，13% partial，429/303 偶发 | 每 6h 全量重抓 + 外企租户量大 | ⏳ 建议：外企 ATS（workday/successfactors/oracle/eightfold/phenom/smartrecruiters）降到每日 2 轮，估省 ~30h/周（18%）；属产品取舍（海外岗新鲜度 6h→12h），待拍板 |
| 每轮都 0 岗的源 25 个 | moka 15 / hotjob 6 …，合计仅 2.1h/周 | 多为对方当前没开岗 | 监控即可，不动 |

## 8. 合规结论（详见 scratch 调研，法条与判例均有来源）

- **爬企业官网公开岗位 = 有条件合法、风险中低**。判据三件套：与被抓方无竞争关系 / 不构成实质性替代（导流回官网）/ 不绕技术措施。刑法 285 条只打「入侵或非公开数据」，不适用。
- 上线前红线：robots 永不绕、不入库任何个人信息、只展示摘要+回链不全文复制、遇登录墙/验证码/签名加密即停不破、按 host 限速并留存「影响评估」记录（《网络数据安全管理条例》第十八条）。
- 建议：UA 标明产品名与联系方式、提供企业「申请移除」入口、保留抓取日志 ≥6 个月、隐私政策写明数据来源。
- 资质：《网络招聘服务管理规定》要求网络招聘服务持人力资源服务许可证；纯聚合跳转是灰色地带，商业化前办证。

## 9. 数据落地与基础设施路线

- **法律强制线**：用户数据在悉尼 = 「向境外提供」。当前 <10 万人且无敏感信息 → 免申报；接近 8 万注册即启动标准合同备案（1~3 个月）。简历/求职意向不属敏感信息。
...
## 11. 第二核心价值（职业洞察）专项审阅（2026-09-03 补）

### 11.1 线上现状（Supabase 实测）

| 指标 | 数值 | 判读 |
|---|---|---|
| active 洞察条目 / 覆盖公司 | 6,248 / 516 | 有岗公司 1,988 家 → 覆盖 26%；画像 1,158 家 → T3 跑过 523 |
| 按维度 | culture 2,544 / hiring 1,176 / path 1,162 / compensation 1,076 / listing 278 / timing 12 | 三主题全堆在经验级 |
| 按来源类型 | community_deidentified 17,896 / public_aggregate 267 / official_site 18 / official_filing 14 | **官方来源 0.2%** |
| 来源域名 top | aiqicha 1,914 / zhihu 1,404 / zhipin 1,120 / nowcoder 1,082 / jobui 950 / instagram 577 / zhaopin 400 / facebook 272 / reddit 196 | 第三方招聘列表页 + 外网社交噪声；143 条 active 的来源全是招聘列表页 |
| 判官 confidence 分布 | 0.9：85%，1.0：13%，pending_review 历史 0 | 判官形式化 |
| sample_size | 95% 非空，中位数 15 | 判官从「N 条评价」抄数，展示成「据约 N 条反馈」误导 |
| T2 事实覆盖 | industry 26 / founded 232 / headcount 63 / hq 211 of 1,158 | Wikidata 对中国公司覆盖极差 |
| 每日吞吐 | 1~4 家 / 天（8 月 25 日后） | 搜索三源日合计 45~60 次，每家 3 主题 × 3 源 = 9 次，再扣校招预留 25 |
| 内容样例 | 腾讯音乐：年终奖「10 个月 / 6~8 个月」具体可读；「加班文化」条目写成音乐氛围 | 主题漂移；比亚迪只有 1 条 Wikidata 上市状态 |

### 11.2 判断

- **架构对，执行错位**：三层（T1 自有派生 / T2 官方事实 / T3 搜索+判官）是正确的合规路线，但供给几乎全压在最贵、最糊的 T3 上，最便宜最可靠的 T1/T2 只用了皮毛。
- **信息面窄且偏**：用户最在意的薪资区间、面试流程、裁员稳定性要么没有、要么被关；事实级的员工规模 / 学历构成 / 人均薪酬对 A 股与非上市公司为零。
- **不是最优解**。最优 = 事实做厚（零边际成本）+ 搜索链只答「只有经验能答」的问题 + 每主题不全源扇出。

### 11.3 本轮执行（代码层，不花钱）

| 批 | 内容 | 预期 |
|---|---|---|
| T3 效率与精度（Codex） | 搜索「首源够用即停」；来源域名黑名单（外网社交 + 第三方招聘列表页）；query 加「公司」限定；默认主题改 年终奖 / 加班 / 面试 / 裁员稳定性；sample_size 原文找不到即置空；判官分档 + evidence_kind 后处理；publisher 按注册域名去重 | 吞吐 ×2~3、噪声源归零、判官有区分度 |
| T1 扩展 + 展示门（Sonnet） | 从岗位库派生经验/学历分布、城市、校招社招实习占比、在架时长；展示门改「sample_size≥5 或 ≥2 域名且 confidence≥0.8」；芯片文案「据 N 个公开来源」 | 1,988 家有岗公司即刻有事实级卡片 |

### 11.4 事实级来源：A 股年报（✅ 已上线 3daf072，首验通过）

- `crawler/official_annual_report.py` + `insight-annual-report.yml`（每日 UTC 20:50，40 家/天）+ 迁移 203。可行性实测见 `2026-09-03-cninfo-annual-report-probe.md`。
- 线上首验：良品铺子（4,665 人、销售 3,904、人均 11.5 万）、郑州银行（6,052 人）、云从科技 均写入；宁德时代 2025 本地端到端 185,839 人 / 技术 12% / 本科及以上 27% / 人均 19.2 万。
- 复盘修掉三处会静默空转的坑：标题高亮 `<em>` 标签导致过滤全漏；「查不到年报」与「已是最新」分开计数；标签后「（人）」括注；学历别名「硕士及以上 / 研究生」。
- 前端走查（用创始人 Chrome 看生产）：抽屉派生统计改两列布局、加载态转圈、日期走 formatDateLabel、散装文字类改语义类。

### 11.5 下一批（待拍板）
- **社保参保人数**（国家企业信用公示 → 天眼查 API，¥0.15/次）：非上市公司体量唯一信号；只对必投清单点查，不全库扫。
- **众包冷启动**（产品决策）：互惠墙（贡献一条解锁全卡）+ 脱敏 offer 截图验证；≥5 条聚合门槛在内测期等于锁死，可先降为 ≥3 并标「样本少」。
