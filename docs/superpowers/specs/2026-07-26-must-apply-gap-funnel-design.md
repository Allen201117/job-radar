...
## 7.3 P1/P2 上线后首轮 prod 实测（2026-07-27）

两轮 apply 实跑（23 家 + 6 家）的结论：

- **验收门经受住了考验**：0 个源入库、0 条脏数据。所有失败都停在台账里。
- **但发现候选入口大面积张冠李戴**（搜索层的病，不是验收层）：东方明珠→MathWorks、利欧集团→闽江学院就业网、华谊兄弟→中国矿大就业网、凤凰传媒→本地宝、掌阅科技→中科创达的飞书租户、博纳影业→省广集团的 hotjob 租户、视觉中国→国聘关键词搜索页。病根 = 候选判定只看「host 是不是已知 ATS + 路径像不像招聘页」，没核验这页是谁家的。
  修法（已上线）：① 硬黑名单（`.edu.cn` 高校就业网 / `bendibao` 等聚合站 / `.gov.cn`；国聘只认 `/job?company=`）② **页面身份门**——fingerprint 本来就会 GET 候选页，顺手核验 title/正文前 3000 字是否出现公司名或其核心词 ③ 逐候选重试（取第一个身份过关且能路由的，不额外烧搜索额度）。
- **反向误杀也修了**：`careers.midea.com` 曾被判「官方信号不足」——路径是根 `/`、公司名「美的集团」又切不出 latin 词。补「招聘子域算强官方信号」（`careers./hr./zhaopin./job(s)./join.` 开头），真正的身份判定交给页面身份门。
- **残留弱点（已知，接受）**：核心词短且撞词时身份门会误过——实测「凤凰传媒」核心词「凤凰」命中了国航招聘页（国航标志就是凤凰）。但它随后卡在「无可用 adapter」，没有进入库。真正的兜底仍是抓取后的健康岗回读 + 标题核验。
- **传媒/文娱是供给现实，不是工具问题**：万达电影/光线传媒/开心麻花/新丽/柠萌/正午阳光/儒意 反复判 `no_official_entry`——它们确实没有公开的逐岗投递入口。这类应走 §4.4 的清单治理，不要靠加工具硬刚。
- **定时 cron 从 dry-run 改为 apply**（2026-07-27）：门已验证安全，dry-run 只写台账不加源，留着当默认等于每天空转。手动 dispatch 仍可传 `apply=false`。
