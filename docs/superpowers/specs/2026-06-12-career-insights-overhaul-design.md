...
> 状态：**P1 / P2 / P3 + 千帆 T3 已全部实现并 commit（2026-06-12）**。P1 派生层 + T2 Wikidata 官方事实已**真机 live 验证**（真 Supabase 32 公司官方事实 + 6 公司派生洞察）；T3 验证引擎 + 千帆检索已建（judge 待 repo 加 `SILICONFLOW_API_KEY` + `BAIDU_QIANFAN_API_KEY` secrets 后 CI 自动跑）。本文件 = 完整设计 + 落地现状。
...
1. ~~T3「公开网络讨论」`百度千帆` 延后~~ → **已接入（2026-06-12）**：`crawler/qianfan_search.py`（口径同 `lib/baidu-qianfan-search.js`）+ `insight_backlog.py --t3`（千帆检索 → 引擎 judge → ≥2 源共识 → 写 `culture`，串行 + 预算守门）+ `insight-enrich-t3.yml`（每日 cron）。**额度**：`qianfan_usage` 表持久当日计数，自封顶 `QIANFAN_DAILY_CAP`（默认 40，留 ~10 给 `/api/discovery`），绝不冲破 50/日；沿用 `BAIDU_QIANFAN_SEARCH_DISABLED` 熔断。**前置**：repo Secrets 需加 `BAIDU_QIANFAN_API_KEY` + `SILICONFLOW_API_KEY`（迁移 137 已加 `qianfan_usage` + `company_profiles.t3_checked_at`）。
