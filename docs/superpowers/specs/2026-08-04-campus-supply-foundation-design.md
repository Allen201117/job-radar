...
- 抓取范围：`board in ('campus','mixed')` ∪ 近 30 天真产过校招岗的源，**且** 公司在必投清单内。
- ⚠️ **只跑 httpx 档，浏览器源不进高频车道**（2026-08-04 实测后修正的设计）：
  选源命中 193 个，其中 **105 个是浏览器串行源**（60 beisen + 45 moka），单源 2-5 分钟
  → 一轮要 3.5~8.7 小时，每小时跑必然撞 50min 超时被杀，既抓不完又天天烧 CI。
  拆分后每小时实抓 **86 个 httpx 源**（分钟级）。浏览器源仍由 `enrich-crawl` 每日全量覆盖，
  覆盖面不减、只是不加密。
  代价可接受的原因：高价值大厂校招板块几乎全在 httpx 档——字节 / 腾讯 / 阿里(×5 BU) / 百度 /
  京东 / 网易 / 美团 / 华为 / 蚂蚁 / 米哈游 / 小红书 / 拼多多 / OPPO / B站 / 携程 / 小米 / vivo。
- **拆分只影响「抓」，不影响「看」**：开闸检测仍对全部 193 个选中源做快照——浏览器源的岗位数
...
1. **立刻重抓该源**：不等下一个整点；走 `tier=all`（允许浏览器档——开闸是罕见事件，值得多花时间）。
   单轮封顶 `SURGE_RECRAWL_CAP=8` 家，按放量幅度排序：旺季多家同时开闸时全跑浏览器档会撞
   50min 超时导致**一家都没抓完**。被推迟的照实打日志（不做静默截断），下一个整点仍是候选
   —— 快照已留痕、基线没被这轮吃掉。
