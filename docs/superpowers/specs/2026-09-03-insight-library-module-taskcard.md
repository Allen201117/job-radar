...
## 8. 第三阶段（2026-09-04）：洞察库改成「只放信息差」

创始人二次定调：**「洞察库先不放数据层了，这种不算啥信息差。用户要的是网上能看到的
大家的经验感受。」** 城市分布 / 学历要求这些用户自己在岗位库筛一下就有。

### 8.1 已做完（全部已上 main 并线上验证）

| 事 | 结果 |
|---|---|
| 洞察库撤掉数据层 | 主体 1,529 → 367（业务线暂时全部消失，见 8.2） |
| 说法层归类（迁移 209） | 6,253 条里 **6,227 条归好类**（99.6%），零 LLM 成本 |
| 说法层质量治理 | 跑题 50.9% → 已清；5,949 → **3,315** 条，覆盖 **454 → 433 家（95%）** |
| 转投救回 | 551 条（薪资 297 / 加班 119 / 实习 57 / 年终奖 35 / 晋升 27 / 面试 16） |
| 规则抽数值 | 419 条（年终奖 182 / 薪资 167 / 面试轮次 70） |
| 档位判档（LLM） | 加班·晋升·实习 1–5 档，走 `insight-grade.yml`，成本约 ¥0.1 |
| **写入端主题门** | `gate_new_claim` 接进 `enrich_company_t3`，答非所问的**根本不写** |
| T3 队列优先级 | 必投清单公司排最前（搜索额度每天只够 ~20 家，要花在用户真会投的公司上） |
| 筛选进 URL | 服务端按 URL 渲染首屏，可分享 / 可收藏 / 刷新不丢 |
| 指标筛选补上限 | 用户要的常是「加班**少**的公司」，只有 ≥ 表达不了 |

### 8.2 仍未做 / 需要拍板

1. **业务线暂时不在洞察库出现**：业务线只有数据层内容，说法层还是公司级的。
   要让它回来，得先做 **T3 按业务线定向检索**（`{公司} {业务线} 加班`）并把结果挂 `subject_id`。
2. **贡献入口聚合门槛仍是 5**（`FIRST_PARTY_MIN_COUNT`）。任务卡建议降到 3，
   但这是**匿名性阈值**、属隐私取舍，我没有单方面改——需要创始人拍板。
   分级验证标记（企业邮箱 / 脱敏 offer / 工牌）同样未做。
2. **趋势指标仍为 0 条**，等 `insight_subject_daily` 快照攒够 30 天自己出现，无需改代码。
   ⚠️ 别改回「用 jobs.first_seen_at 分窗口算」——expired 每日 purge，窗口不完整会把环比算高。
3. **覆盖率**：433 家 / 1,985 家有在招岗的公司 = 22%。提覆盖靠 T3 跑，
   搜索免费额度是硬瓶颈（不是钱：LLM 一天只花 ¥0.085–0.193）。

### 8.3 给下一棒的三条硬提醒

1. **PostgREST 的 `upsert` 不能写部分字段**（会当 INSERT 校验 NOT NULL，整批炸）。
   只改几列一律 `update(...).in_("id", chunk)`，按改动内容分组保住批量。
2. **卡面上给用户看的数字不能取自缓存索引**：索引只管筛选/分面/排序。
3. **缓存索引超 2MB 会被 Vercel 静默丢弃**，症状是每请求重建 ~10s 且不报错。
   改索引结构前先量体积（单测已钉 <1MB）。
