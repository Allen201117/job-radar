...
## 7. 执行记录（2026-08-27，批 3）

拍板结果：**先做批 3**；F-2 用**方案 A**；告警出口用 **GitHub Issue**。

### 7.1 F-1-b 已上线并线上验证 ✅（commit `6cb8605`）

把「取用户小表 → 判 onboarding → 建 feed」整块收进 `loadTodayBundle`，页面组件不再 await 它。

线上实测（`/today?__timing=1`，流式读取，量「页头骨架的字节多久到」）：

| 轮次 | 首字节 | 页头骨架到达 | 差值 |
|---|---|---|---|
| 冷启动 | 1,762ms | 1,774ms | **+12ms** |
| 热 1 | 488ms | 549ms | +61ms |
| 热 2 | 299ms | 312ms | +13ms |
| 热 3 | 295ms | 356ms | +61ms |
| 热 4 | 1,198ms | 1,211ms | +13ms |

**改之前页头骨架不可能早于「首字节 + 用户小表查询」出现**（那 4 条查询实测 1,169ms 挡在返回 JSX 之前）；现在它和首字节几乎同时到。

四件套：node --test 846 通过 / python 1195 通过 / next build 通过 / next lint 无告警。部署 success。

### 7.2 F-1-a 停用词门：**方案被真实数据对拍否决，未上线** ❌

按 §5 批 3 的验收要求做了对拍（10 个真实画像 × 真实 SQL × 香港库），结果**推翻了 §3 F-1-a 的设计**：

| 画像方向 | 原词/扩展词 | 剪掉 | 合格岗 | 展示岗 |
|---|---|---|---|---|
| AI 产品经理 | 8 / 45 | 2 | 165 → 197 **+19%** | 21 → 21 |
| 机械设计实习生 | 3 / 17 | 2 | 25 → 46 **+84%** | 25 → 28 |
| 产品经理 | 5 / 19 | 1 | 358 → 383 +7% | 26 → 26 |
| 数据中台 | 6 / 35 | 2 | 176 → 115 −35% | 24 → 24 |
| 用户运营 | 4 / 25 | 2 | 850 → 526 −38% | 35 → 35 |
| 数据分析 | 4 / 26 | 1 | 368 → 273 −26% | 25 → **23** |
| 设备运维 | 3 / 23 | 3 | 280 → 126 −55% | 25 → 25 |
| **项目专员/助理** | 9 / 42 | **2** | 59 → **19 −68%** | 22 → **16** |

**结论：同一个词对不同用户，一个是噪音、一个是命根子。** 「项目专员/助理」只剪掉 2 个词，合格岗就掉 68%、展示岗掉 6 个。全局停用词表这个设计不成立，**不要再试**（也不要改阈值——2%/1%/0.5% 三档都试过，差别只是伤谁多一点）。

顺带记一个真实但**不可外推**的观察：在那个 28 词的重画像上，剪枝让合格岗 58→126、90+ 分岗 22→51、展示公司 6→12 家。当时差点据此下结论——是多画像对拍拦住了。**n=1 不能下结论。**

### 7.3 F-1 真正的解法（已实测，**需拍板**）

方向层慢的物理原因是：`search_doc` GIN 命中 14.7 万行后，**还要逐行堆扫**才能应用招聘阶段过滤（`lower(title) like any(...)` / `job_type` / `jd_url`），最终只留 7,456 行。这三个 LIKE 走不了索引。

实测把阶段过滤换成可走索引的形式（仅为估时的等价改写）：

| | Execution Time（交替 3 轮） |
|---|---|
| 现状 | 3,465 / 3,839 / 3,586 ms |
| 阶段过滤可走索引 | **626 / 584 / 584 ms（−83%）** |

**与剪枝的本质区别：它改的是物理执行计划，不改召回语义**——同样的岗，只是找得更快。

落地方式（建议）：给 `jobs` 加一个**生成列** `recruitment_scope`（campus / intern / social），由数据库按与现在**完全相同**的表达式算出来，再建索引；召回 SQL 改成 `recruitment_scope = 'campus'`。仓库里已有同款先例：`sources.board`（迁移 187）就是生成列 + 不可变分类函数。生成列由 DB 计算，**爬虫和 app 的写入端都不用改**，天然不会两端漂移。

⚠️ 代价与风险（拍板时要看）：
- 加 STORED 生成列会**重写整张表**（38 万行）并在此期间阻塞写入；香港库是 2C2G/40GB，爬虫是常态高频写入 → 需要挑窗口跑
- 磁盘：新增一列 + 一个索引，需先确认剩余空间
- 走 `gh workflow run jobs-db-migrate`，DDL 不可逆（回滚 = 再发一次 DROP COLUMN）

数据佐证阶段过滤确实是主过滤器：active 381,974 中，校招 66,316（17.4%）、实习 30,007（7.9%）；其中 **42,759 个校招岗只能靠 `jd_url like '%campus%'` 认出来**（标题和 job_type 都没写），所以那个 URL 分支不能删、也不能改用 tsquery 代替。

### 7.4 决策项（新增，替代原 §4 第 5 项之外的内容）

| # | 事 | 选项 | 建议 |
|---|---|---|---|
| 6 | 要不要给 jobs 加 `recruitment_scope` 生成列 + 索引 | 做（召回 −83%，语义不变，但要重写 38 万行的表）/ 不做（/today 继续 3–7 秒）/ 先在低峰期演练一次再定 | **先演练再定**：在低峰窗口跑一次，量实际锁表时长与磁盘增量，再决定是否正式上 |

### 7.5 尚未开工

- 批 1 / 批 2（可观测性）——已拍板方案，等排期
- F-2 方案 A（回填守配额）——已拍板，未实施
- F-4、两个小视觉、批 6 —— 未开工

---

## 8. 决策项 #6 的演练结果：**比预想轻得多，已上线**（commit `d0d58d4`）

拍板是「先演练再定」。演练过程中发现 §7.3 里我建议的**生成列方案是多余的重活**——有个更轻的做法拿到同样的加速：

### 8.1 实际做法

- 加一个 IMMUTABLE 函数 `job_stage_match(title, job_type, jd_url, stage)`，与召回 SQL 的阶段谓词逐字对齐；
- 建两个**分区 GIN**：`jobs_search_doc_campus_gin` / `jobs_search_doc_intern_gin`，谓词就是这个函数。

**不需要生成列、不需要重写表、不需要改任何应用代码。** 关键在于：简单 SQL 函数标了 `immutable` 会被 Postgres **内联**成常量折叠后的 OR-of-LIKE，与召回 SQL 里的 `where` 子句结构完全相同 → 谓词匹配成立 → **现有 SQL 自动改走新索引**（已用 EXPLAIN 确认）。

### 8.2 演练量到的真实代价（这是原本要拍板的东西）

| | 实测 |
|---|---|
| 建索引方式 | `CREATE INDEX CONCURRENTLY`，**全程不锁写** |
| 建索引耗时 | campus 4.5s / intern 6s |
| 磁盘增量 | campus 8.4MB / intern 4.0MB；库 1,736MB → 1,748MB（磁盘 40GB，无压力） |
| 表重写 | **没有**（这正是比生成列方案轻的地方） |

### 8.3 效果与等价性

| 实习档同一条召回（前后对照各 3 轮，**全程未删任何索引**以避免顺序偏差） | |
|---|---|
| 建索引前 | 3,211 / 3,376 / 5,648 ms |
| 建索引后 | **316 / 317 / 482 ms（约 −90%）** |

等价性双证：
1. 函数与现有三连 LIKE **全表 38 万行逐行比对，0 行不符**；
2. 两版召回返回的岗位 id 集合**完全一致**（1800/1800，仅基线有 0、仅新版有 0）。

### 8.4 线上端到端（生产 `/today`，同一账号，5 轮）

| 指标 | 本轮开工前 | 现在 |
|---|---|---|
| 页头骨架到达 | ≥ 首字节 + 1,169ms | **296–536ms** |
| 召回 `recall` | 6,764ms | **603–977ms** |
| 整页流关闭 | 11,871ms（本轮首测）/ 7,670ms（交接单） | **2,455–3,152ms** |
| 展示岗位 / 候选数 | 30 / 1,214 | 30 / **1,214**（完全一致 → 召回语义确实没动） |

### 8.5 留下的哨兵与已知边界

- `tests/recall-stage-index-alignment.test.js`：一旦 `stageRecallPatterns` 与 schema 里的谓词不再逐字一致，planner 会**静默**不用这个索引（不报错、不出错，只是又慢回去）。已验证注入漂移时该测试确实 FAIL。
- **社招用户（无阶段过滤）用不到这两个索引**，仍走全量 `jobs_search_doc_gin`。已知边界，不是漏配。
- 现在的大头变成 `compute`（1,198–1,342ms，1,214 个候选逐岗 JS 打分）。要继续压首屏，下一刀在这里。

### 8.6 §7.3 的建议作废

原文建议的「生成列 + 重写 38 万行表」**不要做**——演练证明分区索引已经拿到同样的收益，代价小两个数量级。决策项 #6 到此关闭。

---

## 9. 全部批次执行结果（2026-08-27 收尾）

四个包并行执行，全部上线并逐页线上验证。另加了两项计划外的改动（§9.5、§9.6）。

### 9.1 批 1 看板说真话 ✅（`20e35d5`）

线上实测：「系统运行 **6/6 · 全部正常**」→「后台产出 **6/8** · ✕ 处理 · 1 个出问题：刷新/发现」；「全部正常」这个词从页面上消失。模块卡 6 张 → 8 张（新增「缺口漏斗」「校招供给」）。每张卡两个信号分开显示：「有产出 / 产出为 0」与「跑 N · 挂 M」。原来那格「产出岗位 0 · ● 正常」现在是「刷新/发现 · 0 · ✕ 产出为 0」。

先演练再定（14 天真实台账喂改好的代码）：112 个「卡×天」格，绿 70 / 黄 27 / **红 3** / 灰 12——红仅 2.7%，没有把一切判红；热力图老判据红 13 格 → 新判据红 0 格黄 14 格。

判据优先级最终定为：无记录 → 全挂 → **产出 0** → 有失败 → 健康。「产出 0」故意排在「有失败」之前（跑一整天零产出比「跑 10 次挂 1 次」严重）。同时删掉了 `dailyTrackerTone`（那第二套判据）并加反向哨兵测试钉死。

### 9.2 批 2 真产出告警 ✅（`b294f3d`）

`crawler/ops_watchdog.py` 五条规则 + `ops-watchdog.yml`（每日 UTC 01:00，**默认 dry-run**，`OPS_WATCHDOG_APPLY=true` 才真开 issue）。`db-report.yml` / `production-smoke.yml` 补上 schedule。

**两处判据被真实数据改掉了**（原方案照写会「一条都告不出来」或「天天误报」）：
- 规则 B：只认「≥95% 声明 timeout」实跑零命中——dead-link-audit 分片每晚在**第 90 分钟整**被杀，而它声明 150 分钟。补了「同一 workflow 反复出现 cancelled job」判据。
- 规则 E：下限 3h → 24h，否则 GitHub 丢 schedule 触发的已知抖动会天天误报。

生产 dry-run 开出 **10 条告警全部是真问题**（详见 commit）。其中一条值得单独看：**近 48 小时 10 条 `llm_insufficient_balance` 埋点是真实用户传简历时撞上 LLM 402、静默降级到规则解析**——这是正在发生的产品降级。

### 9.3 F-2 公司配额 ✅（`4a3cd89`）

回填不再无条件撤销配额（上限放宽到 `cap + floor(cap/2)`）。⚠️ **critical 区一度按方案加了配额，随即撤回**：那里是「你收藏的岗关闭了」的告警不是发现流，限流等于吞掉用户自己的东西。已加反向哨兵测试 + 注释「别再加」。

### 9.4 F-4 卡片瘦身 ✅（`0273928`）——**未达验收标准，如实记录**

JD 收 1 行、5 按钮收成 2+更多、卡片去蓝（线上实测 `blueLeft: 0`）、落地页孤字修好（两行）。深色主题已验。

**但没达到 §5 批 5 写的「页面高度 ≥40%」**：桌面端卡片高度取「左栏 vs 右栏」最大值，左栏本来就比按钮栏高，所以按钮收敛几乎不降卡高；降幅几乎全来自 JD 收 1 行（桌面 −8~10%，手机 −15~21%）。**要真正解决「滚 14 屏」得做分段加载**，本轮按计划没碰。

⚠️ **测量方法论的坑**：这一页卡片用 `content-visibility: auto` + `contain-intrinsic-size: auto 340px`，**屏幕外的卡报的是「记住的尺寸」而不是真高度**，所以 `document.documentElement.scrollHeight` 不能用来比较改动前后。可见卡的真实高度是 311px。以后量这一页的高度必须先滚一遍或只量可见卡。

### 9.5 计划外①：召回跨职能剪枝 ✅（`2ca9c58`）

`compute` 复测时发现的（§3 F-1-c 原本说「等召回改完再测」）：召回 1,216 个候选里 **1,053 个被方向门拒掉，光「研发」职能就 724 个**——用户要的是产品岗，而召回把整个研发词库拉了进来。

修法：召回构造 tsquery 时用**同一套** `userTargetFunctions` + `classifyJobFunction` 剪掉跨职能扩展词（用户原词永不剪、职能判不出的词保留、没填 targetRoles 时一个都不剪）。

33 个真实画像对拍：合格岗 +20.2%、90+ 分岗 **+86.4%**、**展示岗没有一个画像变少（0/11）**，另 22 个画像根本没有可剪的词、零影响。

**同时正式作废 §7.2 的「全局通用词表」方案。** 另外：§7.2 那份对拍数据出自一个有缺陷的 harness（用字符串替换生成变体 SQL，会切坏 tsquery 字面量，且旧版没开 `ON_ERROR_STOP` 把 psql 报错吞了）。干净 harness 改成**在 params 数组里按值精确定位替换、定位不到就大声跳过**。复验只跑到 2 个画像（数字与旧结果一致）就被职能剪枝取代，故 §7.2 的结论应视为**未完全复验**——但结论方向不影响决策，因为职能剪枝在每个指标上都更好且更安全。

### 9.6 计划外②：分区 GIN（见 §8）

### 9.7 线上端到端（生产 `/today`，同一账号）

| 指标 | 本轮开工前 | 收尾时 |
|---|---|---|
| 页头骨架到达 | ≥ 首字节 + 1,169ms | 296–536ms |
| 召回 | 6,764ms | 603–977ms |
| 整页流关闭 | 11,871ms | 2,455–3,152ms |
| 合格岗 | 58 | 93 |
| 展示公司数 | 6 | **9** |
| 最大单公司占比 | 字节 15/30 = **50%** | 字节 11/30 = **37%** |

### 9.8 遗留（下一轮的输入）

1. **`/today` 滚 14 屏没解决**——要做分段加载（F-4 只降了单卡高度）。
2. **公司别名不合并**：「携程集团 Trip.com」与「携程」被算成两家、各吃一份配额。影响面不止配额（logo / 洞察 / 必投清单匹配都吃它）。
3. **多岗位串没切分**（既有 bug）：有用户把目标岗位填成 `"销售；采购"` 一个字符串，`classifyJobFunction` 对整串只认出「销售」，线上职能门本来就已把该用户的所有采购岗拒光。
4. **产出口径有两份**：看板走 `lib/admin-health.ts` 的 `moduleVerdict`，watchdog 走 `crawler/ops_watchdog.py` 的 `MODULE_OUTPUT`。TS/Python 两端，改口径要同步——长期应像 `must-apply-list.json` 那样共读一份。
5. **搜索 provider 没有账户级判据**：`crawler/search_provider_http.py` / `qianfan_search.py` 对 `status >= 300` 一律 print 一行返回 `[]`，欠费和「没搜到」长得一模一样、也不落库。
6. **`dead-link-audit` 第 90 分钟被杀的凶手没查出来**（不是 timeout、不是 concurrency、仓库里搜不到 cancel）。watchdog 只负责把它告出来。
7. **仓库 Actions 默认 token 权限是 read**：第一次开 `OPS_WATCHDOG_APPLY=true` 要盯一眼，若 403 需去 Settings 改成 Read and write。
8. **`compute` 仍是 1.2–1.4s**（1,200 候选逐岗 JS 打分），是现在 `/today` 的下一个大头。
