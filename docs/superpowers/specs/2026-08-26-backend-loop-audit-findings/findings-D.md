...
- 日吞吐 11,800/天（07-28 前）→ 1,900-7,800/天（日均 ~4,700）。时间点与 commit 29c43c9（按源摊名额）+ --must-apply-first 上线重合——按源摊名额后候选散布到大量慢渲染租户，1,500 岗/90min（~3.6s/岗）不够。**「CI 绿灯≠有产出」现行案例：run 列表里是 cancelled，没人看。**
...
### 系统性「暗死」存量（信号盲区，必须单列）
1. **wecruit「渠道未发布」存量未动**（11725ee 只修增量）：新城控股 1,219 + 歌尔 468 = 1,687 条仍 active、100% 72h 内被盖「已检」章；真浏览器实测无限转圈。荣耀 411 条同病同批。合计 ~2,378。
