...
function loadAvailabilityRoute({ storeEnabled, storeCounts, rpcCounts } = {}) {
  const calls = { store: 0, rpc: 0 };
  const supabase = {
  ...
  const route = loadRoute("app/api/insights/availability/route.ts", {
    ...
    "@/lib/insight-match": { findCompanyProfile: () => PROFILE },
    "@/lib/insight-bundle": {
...
test("availability uses HK company counts and skips Supabase RPC when jobs store is enabled", async () => {
  ...
  assert.deepEqual(calls, { store: 1, rpc: 0 });
});
...
test("availability uses Supabase RPC when jobs store is disabled", async () => {
  ...
  assert.deepEqual(calls, { store: 0, rpc: 1 });
});
