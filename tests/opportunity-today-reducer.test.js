...
// v3 五分区；这些 reducer 测试只关心「主清单」的乐观增删，放在 main 区。
function sections(main = []) {
  return { critical: [], main: main.map(opp), explore: [], momentum: [], waiting: [] };
}
...
test("finalizeRemove 落定（保持移除、清 pending/toast）", () => {
  ...
  assert.deepEqual(ids(s.sections.main), ["b"]);
...
test("undo 成功：恢复后 commit", () => {
  ...
  assert.deepEqual(ids(s.sections.main), ["a", "b"]);
...
test("多岗并发互不影响", () => {
  ...
  assert.deepEqual(ids(s.sections.main), ["a", "b"]); // a 回原位 index0；c 仍移除
