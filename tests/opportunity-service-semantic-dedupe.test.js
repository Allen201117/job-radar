...
test("service 后追加的外部 critical 按语义取代另一 ID 的普通分区岗位", async () => {
  ...
  const { buildOpportunityFeed } = loadTsWithMocks(path.join(ROOT, "lib", "opportunities", "service.ts"), {
    ...
    "../jobs-store/opportunities": {
      recallOpportunityCandidates: async () => ({ jobs: [mainJob], capped: false }),
    },
    "../jobs-store/read": {
  ...
  const feed = await buildOpportunityFeed(
    ...
    [{ job_id: "critical-id", action: "saved" }],
    null,
