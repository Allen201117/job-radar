...
// ✅ 这里的绕法：动态请求里让缓存调用脱离当前 work unit（workUnitAsyncStorage.exit），条目名退回不带网址的形式。
...
// ⚠️ `.external` 结尾的 Next 内部模块永远不被打包、与运行时共享同一个实例（Next 自己就靠它共享请求上下文），
//    所以这里 exit 的就是 unstable_cache 读的那一个。改 import 路径前先看 node_modules/next/dist/build/handle-externals.js。
...
export function requestSafeCache<T extends AnyAsyncFn>(cb: T, keyParts?: string[], options?: CacheOptions): T {
  ...
  const safe = (...args: Parameters<T>) => {
    const store = workUnitAsyncStorage.getStore();
    if (store?.type === "request") return workUnitAsyncStorage.exit(() => cached(...args));
    return cached(...args);
  };
  return safe as T;
}
