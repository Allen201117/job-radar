...
// 命中用户偏好排除词的岗位一律剔除（子串小写匹配 标题/公司/地点/类型/摘要/薪资）。
// 逐字对齐 crawler/discovery.py 的 job_excluded，保证同步刷新与后台抓取口径一致；空列表=不过滤。
function jobExcluded(job, exclude) {
  const terms = (exclude || [])
    .map((t) => String(t == null ? "" : t).trim().toLowerCase())
    .filter(Boolean);
  if (terms.length === 0) return false;
  const hay = [job.title, job.company, job.location, job.job_type, job.summary, job.salary_text]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
...
function excludeJobs(jobs, exclude) {
  return (jobs || []).filter((job) => !jobExcluded(job, exclude));
}
