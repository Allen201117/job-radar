...
function makePool(): Pool {
  ...
  const pool = new Pool({
    ...
    // ⚠️⚠️ 别再把这个值调小。2026-09-03 试过 5→2，**生产实测当场打成事故**：
    // 单发仍 200/0.9s，但 20 并发 **20/20 全返 500、每个卡满 52s**（改前同样 20 并发是全部 200、
    // p50 1.24s）。改回 5 后并发立刻恢复。
    // 教训：当时的推理前提「Node 函数一个实例同时只处理一个请求，所以 2 条够用」是**错的**——
    // 这个平台上一个实例会并发处理多个请求，池子一小就互相抢连接、排队雪崩。
    // 想省连接位不能从这里砍：真正的大头是 CI 爬虫（21:00 UTC 撞档时 ~73 条 / 100），
    // 该去限爬虫并发或上 PgBouncer，不是缩函数池。
    max: 5,
