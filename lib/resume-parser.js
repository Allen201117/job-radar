...
// ⚠️ 技能 ≠ 关键词，别再把 skills 灌进 target_keywords（2026-09-02 定，踩过两次坑）。
// 语义分工（见 supabase/migrations/202_user_preferences_skills.sql）：
//   target_roles    = 想做什么 → 决定召回与方向判定
//   skills          = 会什么   → 只参与打分加分，永不判方向
//   target_keywords = 用户自己在偏好页补充的搜索词，**简历解析不写它**
// 旧实现 `target_keywords: profile.skills` 把「会什么」贴上了「要搜什么」的标签，害得
// /jobs 的默认筛选词被填成「用户旅程」（恒 0 结果），/today 的召回被技能词稀释掉近九成候选。
function buildPreferencesFromResumeProfile(profile) {
  return {
    ...
    skills: unique(profile?.skills || []).slice(0, 16),
    // 行业带入偏好：跨行业门要用（公司→行业 ∈ 用户目标行业才算命中），见 lib/company-industry。
    industries: unique(profile?.industries || []).slice(0, 6),
  };
}
