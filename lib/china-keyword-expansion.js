...
function _titleRoleClusterConflict(job, query) {
  ...
  for (let i = 0; i < CHINA_KEYWORD_GROUPS.length; i++) {
    ...
    if (_groupRoleTerms(i).some((term) => containsTerm(titleText, term))) return true;
  }
  return false;
}
...
const ROLE_FILLER_SUFFIX_RE = /(相关|方向|岗位|职位|工作|类|岗)$/;
...
function normalizeRolePhrases(roles) {
  ...
  for (const raw of Array.isArray(roles) ? roles : [roles]) {
    for (let part of String(raw || "").split(/[/／|｜]/)) {
      part = part.trim();
      if (!part) continue;
      let stripped = part.replace(ROLE_FILLER_SUFFIX_RE, "").trim();
      stripped = stripped.replace(ROLE_MODIFIER_PREFIX_RE, "").trim();
