...
function jobMatchesChinaKeyword(job, query) {
  ...
  // AND across 单元，OR within 单元 → 组合意图精准（"AI PM" = AI ∧ 产品）。
  return units.every((unit) => unit.some((term) => containsTerm(searchable, term)));
}
...
const JOB_FUNCTION_RULES = [
  ["产品", /产品经理|产品运营|产品策划|产品设计师|product\s*manager|\bpm\b|\bpo\b|产品/i],
