...
// ── 匹配层记忆化（纯性能，语义零变化）──────────────────────────────────────────
// 一次无筛选的 /api/jobs/search 要给 SCAN_BUDGET=28000 个候选岗位逐个打分，而打分对
// **每个岗位 × 用户每个 target_keyword** 都重跑一遍 keywordMatchUnits / _matchedGroupIndexes。
// 这两个函数只依赖「查询串（+ 是否含海外词库）」，与岗位无关，却被重算了 28000 次：用户存了
// 23 个关键词时，光这两处就是 28000 × 23 次「遍历 46 个概念组 × 组内全部词」的纯重复计算。
// 2026-09-02 线上实测：某画像（23 个 target_keywords）清空筛选后的搜索耗时 60,149ms，
// 撞上 app/api/jobs/search/route.ts 的 maxDuration=60 被杀 →「搜索失败，请重试」。
// 缓存键就是函数的全部入参，因此命中与否结果逐字节相同；返回值是只读的（全部调用方只做
// every/some/filter/map，见 lib/jobs-store/opportunities.ts:118 与本文件内各处），别改成就地改写。
const MEMO_MAX = 1024;

/**
 * 带上限的按键记忆化。泛型标注是必需的：本文件是 .js，TS 端（lib/jobs-store/opportunities.ts）
 * 靠推断拿 keywordMatchUnits 的返回类型，缺了它整条链会退化成 any 并让 build 的类型检查失败。
 * @template T
...
function memoize(cache, key, compute) {
  ...
  const value = compute();
  // 长驻 lambda 里查询串种类有限，但仍加个上限兜底：超了整片丢弃，重新攒。
...
const keywordMatchUnitsCache = new Map();
/** @type {Map<string, number[]>} */
const matchedGroupIndexesCache = new Map();
...
function keywordMatchUnits(query, options = {}) {
  const raw = String(query || "").trim();
  if (!raw) return [];
  // 入参只有 query 与「是否并入海外词库」，两者进键 → 缓存命中与否结果完全一致（见上方注释）。
  return memoize(
    keywordMatchUnitsCache,
    `${shouldIncludeOverseasLexicon(options) ? "1" : "0"} ${raw}`,
    () => _computeKeywordMatchUnits(raw, options),
  );
}

function _computeKeywordMatchUnits(raw, options) {
...
function _matchedGroupIndexes(query) {
  ...
  // 只依赖归一后的查询串 → 按它缓存（每岗每关键词都会问一次，见文件上方记忆化说明）。
  return memoize(matchedGroupIndexesCache, normalized, () => {
    const idxs = [];
...
// 短拉丁词的词边界正则：词表是固定的，但 containsTerm 每岗每词都会被调到，
// 每次 new RegExp 等于把同一个模式重复编译上百万次 → 编译一次存下来复用。
/** @type {Map<string, RegExp>} */
const shortLatinTermPatterns = new Map();

function shortLatinTermPattern(t) {
  return memoize(shortLatinTermPatterns, t, () => {
...
function containsTerm(haystack, term) {
  ...
  if (/^[a-z0-9.+#-]{1,3}$/.test(t)) {
    return shortLatinTermPattern(t).test(h);
  }
