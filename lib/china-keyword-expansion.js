...
const JOB_FUNCTION_RULES = [
  ...
  ["产品", /产品经理|产品策划|产品负责人|产品总监|产品专家|product\s*manager|product\s*owner|product\s*lead|(?:director|head|vp|vice\s*president)[,\s]+(?:of\s+)?product/i],
...
// 这些桶的关键词在 JD 正文里是万能套话（研发：技术/开发；项目管理：项目管理经验），
// 正文兜底判出它们等于没判：研发会把标题未判出的岗泛化成技术岗，项目管理会把海量岗位误标成
// 项目管理。两者一律退回标题结论，只有标题里的明确角色词才能归入对应桶。
const BODY_FALLBACK_BLOCKED = new Set(["研发", "项目管理"]);
