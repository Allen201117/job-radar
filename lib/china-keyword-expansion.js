...
function normalizeChinaJobType({ title, sourceType, url, summary } = {}) {
  ...
  if (/校招|校园招聘|应届|毕业生|campus|new\s+grads?\b|\bgraduate\b|xiaozhao/.test(text)) return "校招";
...
function recruitmentCategory(job = {}) {
  ...
  // 硬门（最高优先级）：JD 明确要求 ≥2 年经验 → 强制社招。纠正两类误判：
  // ① JD 正文里"毕业生/应届/graduate/campus"等字样把社招岗污染成校招；
  // ② 源 job_type 本身把资深岗错标成"校招"（如光刻主任工程师 job_type=校招）。
  ...
  if (/实习|\bintern(?:ship)?s?\b/i.test(specific)) return "实习";
  if (/校招|校园|应届|毕业生|campus|new\s?grads?\b|\bgraduate\b|管培|管理培训生|留学生|overseas student/i.test(specific)) {
    return "校招";
  }
...
function hasExplicitRecruitmentType(job = {}) {
  // ≥2 年经验硬要求本身就是明确的社招信号 → 让"筛校招"能真正淘汰这类岗（而非当类型未知放行）。
