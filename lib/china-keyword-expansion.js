...
const CHINA_KEYWORD_GROUPS = [
  ...
  [
    ...
    // 「语音合成大模型」等 AI 方向噪音，未收录；「合成高级研究员/合成助理研究员」因中间插了职级词
    // 不含连续子串「合成研究员」，属已知残留缺口（不复活裸「研究员」，见下方 2026-09-17 教训）。
    "药物化学",
    "制剂",
    "药理",
    "原料药",
...
function recruitmentCategory(job = {}) {
  ...
  // 荣耀应届生门户 93 岗进库后 91 个判成社招就是这么来的。这个参数是门户自己声明的渠道，与路径令牌同级可信。
  ...
  // 这就是同一个失效模式当场发生的样子。与上面的 postType 令牌同级可信。
  ...
  if (
    ...
    /[?&]postType=intern(?:s|ship)?\b/i.test(url) ||
    /[?&]recruitType=12(?![0-9])/i.test(url)
  ) {
    return "实习";
  }
  ...
  if (/[?&]postType=campus\b/i.test(url)) return "校招";
  ...
  if (/[?&]recruitType=1(?![0-9])/i.test(url)) return "校招";
...
const JOB_FUNCTION_RULES = [
  ...
  ["设计", /视觉设计|交互设计|ui\s*设计|ux|平面设计|设计师|美工|美术设计|原画|三维建模|3d\s*建模|建模师|blender|designer/i],
  ...
  // 「实验员」的前缀排除是必须的：药理/药物/医学/中药 系的实验员属医疗健康，而中文按最靠后命中，
  // 「药理实验员」里的「实验员」位置更靠后会把它从医疗健康抢走（全库实测 14 个岗）。
  ["生产制造", /检修|巡操|集控|普工|维保|(?:维修|设备|机修|喷漆|钣金|安装)技师|(?<!药理|药物|医学|中药|临床|生物|毒理|动物|筛选|检验|发现)实验员|化验员|有机合成|设备管理|设备主管|精益|(?<!网络|信息)安全管理|安全生产|electrician|welder|mechatronic\w*|robotics\s*tech/i],
