...
function cityMatchTokens(city) {
  ...
  if (!raw) return [];
  const canonical = normalizeChinaCity(raw);
  const set = new Set([normalizeForMatch(raw), normalizeForMatch(canonical)]);
...
function recruitmentCategory(job = {}) {
  ...
  if (/\/(social|experienced)(\/|\?|$)/i.test(url)) return "社招";
...
const JOB_FUNCTION_RULES = [
  ...
  ["产品", /产品经理|产品运营|产品策划|产品负责人|产品总监|产品专家|product\s*manager|product\s*owner|technical\s*program\s*manager|\btpm\b|\bpm\b|\bpo\b/i],
