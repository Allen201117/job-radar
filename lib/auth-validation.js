...
function validateOtp(code) {
  // Supabase 邮箱 OTP 长度可在后台配置（本项目实测 signup 确认码为 8 位，recovery 可能不同）。
  // 放宽到 6–8 位数字以兼容各流程；最终真伪由服务端 verifyOtp 判定。
  if (!/^\d{6,8}$/.test(String(code || "").trim())) return "请输入验证码（6–8 位数字）";
  return null;
}
