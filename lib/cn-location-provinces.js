...
//   ⑤ 紧跟「[方位]路 / [方位]街 / 大道 / 大街」是街道名（南京路 / 中山路 / 南京东路 / 延安东路 / 深圳大道）：
//      上海大量街道以外省地名命名，「黄浦（延安东路518号）」只认「路 / 街」时会判给陕西（2026-09-23 全库唯一一例，
//      2 行）。「广州南站」「营口站前区」的方位字 / 站不是街道，照认。
...
// 规则⑤：地名后面紧跟这些 = 街道名。与 crawler/geo.py 的 _STREET_AFTER_PLACE_RE 逐字相同。
const STREET_AFTER_PLACE_RE = /^[东西南北中]?(?:路|街|大道|大街)/;
...
function segmentProvince(seg) {
  for (let i = 0; i < seg.length; i++) {
    ...
    if (STREET_AFTER_PLACE_RE.test(seg.slice(i + name.length))) continue;
...
module.exports = {
  ...
  CN_MUNICIPALITIES: CN_PROVINCE_DATA.municipalities,
  locationProvinces,
