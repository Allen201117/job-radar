...
const { classifyJobFunction, normalizeRolePhrases } = require("./china-keyword-expansion");
const { normalizeCityPhrases } = require("./profile-hygiene");
...
- target_roles：输出求职者想做的岗位方向，最多 3 个，按意愿强弱排序；不要输出技能、行业或公司名。
  ⚠️ 编程语言 / 框架 / 工具名（Java、Python、React、Spring、SQL、Excel、Figma）是技能，**绝不能**写进 target_roles；
  简历只写了技能没写意向时，按经历推断岗位方向（如「Java 后端开发工程师」），推断不出就给空数组。
  ⚠️ **必须是干净的岗位名**，一个元素只放一个岗位：
  · 不要塞两个岗位（写「项目专员」「助理」两条，不要写「项目专员/助理」「销售；采购」）；
  · 不要带「相关 / 岗位 / 职位 / 方向 / 类 / 工作」这类后缀（写「测试」不要写「技术岗位」「英文相关」）；
  · 不要带「办公室 / 门店 / 总部」这类地点修饰和括号补充（写「文员」不要写「办公室文员」，
    写「机械工程师助理」不要写「机械工程师助理（设计方向）」）；
  · 不要把求职阶段写进岗位名（写「机械设计」不要写「机械设计实习生」，阶段归 experience_stage 管）。
- target_locations：**标准城市名**，一个元素一个城市（写「杭州」「深圳」两条，不要写「杭州 深圳」）。
  简历写的是省/城市群（广东、长三角）就原样保留省名，不要自行猜成某个市。
...
// 目标岗位里的「技能词」门（2026-09-17 加）：prompt 已禁止把 Java / Python 写进 target_roles，但模型不总听话，
// 而 target_roles 是方向判定的唯一依据（lib/scoring.ts / eligibility.ts）——「Java」当方向会让所有含 Java 的岗
// 拿到方向命中分。判据取交集：这个词**同时**出现在 skills 里、且按标题判不出职能（classifyJobFunction → 其他），
// 才认定它是技能词。只满足一条不删：「AI Agent」判不出职能但不在技能表里，是合法的纯领域方向词；
// 「数据分析」既在技能表又判得出职能（数据），也是合法方向。无从判断就别猜。
function dropSkillWordsFromRoles(roles, skills) {
  const skillKeys = new Set((skills || []).map((s) => String(s || "").trim().toLowerCase()).filter(Boolean));
  if (!skillKeys.size) return roles;
  return roles.filter((role) => {
    const key = String(role || "").trim().toLowerCase();
    if (!skillKeys.has(key)) return true;
    const fn = classifyJobFunction({ title: role });
    return Boolean(fn && fn !== "其他");
  });
}
...
function normalizeResumeProfile(raw) {
  ...
  const skills = normalizeSkillTokens(data.skills);
  ...
  return {
    ...
    // prompt 是软约束，模型照样会吐「行政/后勤类」「机械工程师助理（设计方向）」「机械设计实习生」
    // 这类**带修饰的复合写法**（2026-09-17 存量体检在 candidate_profiles 里逐条见到）。
    // 这里做代码级保证：与手填入口（preferences-input）走同一份归一，档案与偏好两处口径不会漂。
    target_roles: uniqStrings(
      normalizeRolePhrases(dropSkillWordsFromRoles(uniqStrings(data.target_roles, MAX_TAGS), skills)),
      MAX_TAGS,
    ),
    target_locations: uniqStrings(normalizeCityPhrases(uniqStrings(data.target_locations, MAX_TAGS)), MAX_TAGS),
    skills,
