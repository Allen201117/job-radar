...
const COUNTRY_TOKENS = {
  // ⚠️ TW 必须排在 CN 前面（对象按插入序遍历，先命中先返回）。
  // 台湾按项目口径「不抓、不归入任一范围」：TW ∉ 任何 source regions 且 TW ∉ GREATER_CHINA。
  // 这一条 Python 侧 2026-07-28 就补了，JS 侧一直缺 —— "Taipei, Taiwan, Province of China"
  // 因为含 "china" 在 JS 里被判成 CN。两边必须逐条一致。
  TW: ["taiwan", "台湾", "臺灣", "taipei", "台北", "臺北", "kaohsiung", "高雄", "hsinchu", "新竹"],
  // ⚠️「新界」「九龙」不能放这里：本表的中文 token 是**子串**匹配，重庆有九龙坡区
