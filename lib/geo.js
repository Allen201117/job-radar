...
// 切段正则与 segments() 定义在 ./cn-location-provinces.js（省级解析要在浏览器端复用，别为它把整份 geo.js 打进前端包）。
const { SEGMENT_SPLIT_RE, segments } = require("./cn-location-provinces");
...
// ⚠️ 与 crawler/geo.py 的 _OVERSEAS_UNSPECIFIED_SEGMENTS 逐条一致。
// 「X区域」与「X地区」同构：中控技术把 26 个在招岗写成「海外区域」，补前落回 source.regions
// 兜底被 CN 源判成 domestic（2026-09-05 全库扫描，自报境外却判 domestic 的只剩这一种写法）。
const OVERSEAS_UNSPECIFIED_SEGMENTS = new Set([
  "国外", "国外地区", "国外区域", "境外", "境外地区", "境外区域",
  "海外", "海外国家", "海外地区", "海外区域",
]);
...
// ---------------------------------------------------------------------------
// 标题里的城市（2026-09-23）：location 为空时的兜底，写库时物化进 location（lib/jobs-store/write.ts）。
// ⚠️ 与 crawler/geo.py 的 title_city_location 逐条镜像，两侧共读 tests/fixtures/title-city-cases.json，
// 下面四张表由 tests/geo.test.js 与 geo.py 逐条对拍。为什么这么判（实测数字、五条「宁可漏判」取舍）
// 写在 geo.py 同名函数上方，此处不重复。
// ---------------------------------------------------------------------------
const TITLE_CITY_PROVINCE_NAMES = [
  "河北", "山西", "辽宁", "吉林", "黑龙江", "江苏", "浙江", "安徽", "福建", "江西", "山东", "河南", "湖北", "湖南",
  "广东", "海南", "四川", "贵州", "云南", "陕西", "甘肃", "青海", "台湾省", "内蒙古", "广西", "西藏", "宁夏", "新疆",
  "内蒙古自治", "广西壮族", "西藏自治", "宁夏回族", "新疆维吾尔",
];
const TITLE_CITY_VETO_SEGMENTS = [
  "全国", "全国各地", "各地", "多地", "异地", "不限", "地点不限", "不限地区", "全部地区", "全球", "全球各地",
  "其他地区", "其它地区",
  "海外", "国外", "境外", "海外地区", "国外地区", "海外国家", "境外地区", "海外区域", "国外区域", "境外区域",
];
const TITLE_CITY_SUFFIX_ONLY_NAMES = ["阿里", "朝阳", "东方", "白沙", "新星", "双河"];
const TITLE_CITY_SUFFIXES = ["自治州", "地区", "新区", "市", "州", "盟"];

// 省 → 地级短名：与省级解析（./cn-location-provinces.js）共读同一份 JSON，规则 ③ 的「省市对得上」靠它判。
const { CN_PROVINCE_PREFECTURES: TITLE_PROVINCE_PREFECTURES } = require("./cn-location-provinces");
const TITLE_CITY_PROVINCE_SET = new Set(TITLE_CITY_PROVINCE_NAMES);
const TITLE_CITY_VETO_SET = new Set(TITLE_CITY_VETO_SEGMENTS);
const TITLE_CITY_SUFFIX_ONLY_SET = new Set(TITLE_CITY_SUFFIX_ONLY_NAMES);
const TITLE_ORG_PAREN_RE = new RegExp(
  "[（(][^（）()]{1,8}[）)](?=[\\u4e00-\\u9fff]{0,16}?" +
    "(?:有限责任公司|股份有限公司|有限公司|分公司|公司|集团|研究院|研究所|设计院|工程局|医院|银行))",
  "g",
);

function titlePlace(seg) {
  if (CN_ADMIN_NAME_SET.has(seg)) return TITLE_CITY_SUFFIX_ONLY_SET.has(seg) ? null : seg;
  for (const suffix of TITLE_CITY_SUFFIXES) {
    if (seg.endsWith(suffix) && seg.length > suffix.length) {
      const name = seg.slice(0, -suffix.length);
      if (CN_ADMIN_NAME_SET.has(name)) return name;
    }
  }
  return null;
}

// 省级名（含「内蒙古自治」长写法）→ 映射表里的省短名；台湾省等不在表里的返回 null。
function titleProvinceKey(place) {
  for (const province of Object.keys(TITLE_PROVINCE_PREFECTURES)) {
    if (place.startsWith(province)) return province;
  }
  return null;
}

// 「江苏南京」「江苏省南京市」「辽宁朝阳」→ [省, 地名]；只拆不判，省市对不对得上由 titleCityLocation 统一检查。
function titleGluedPlace(seg) {
  for (const province of Object.keys(TITLE_PROVINCE_PREFECTURES)) {
    if (seg.length <= province.length || !seg.startsWith(province)) continue;
    let rest = seg.slice(province.length);
    if (rest.startsWith("省")) rest = rest.slice(1);
    const place = titlePlace(rest) || (CN_ADMIN_NAME_SET.has(rest) ? rest : null);
    if (place) return [province, place];
  }
  return null;
}

// 从岗位标题认出工作城市，给 location 为空的岗兜底；认不出 / 不该填返回 null。
// 多个城市按标题顺序用「/」连接。只在 adapter 没给地点时用，绝不覆盖 adapter 的地点。
function titleCityLocation(title) {
  if (!title) return null;
  const text = String(title).replace(TITLE_ORG_PAREN_RE, " ");
  const cities = [];
  const provinces = [];
  for (const seg of segments(text)) {
    if (TITLE_CITY_VETO_SET.has(seg)) return null;
    let place = titlePlace(seg);
    if (place === null) {
      const glued = titleGluedPlace(seg);
      if (glued === null) continue;
      provinces.push(glued[0]);
      place = glued[1];
    } else if (TITLE_CITY_PROVINCE_SET.has(place)) {
      const province = titleProvinceKey(place);
      if (province === null) return null;
      provinces.push(province);
      continue;
    }
    if (!cities.includes(place)) cities.push(place);
  }
  if (provinces.length) {
    // 规则 ③：每个城市都落在出现过的某个省里，每个省都有本省城市，否则是跨省区域岗或省市打架。
    const covered = new Set();
    for (const city of cities) {
      const owner = provinces.find((p) => TITLE_PROVINCE_PREFECTURES[p].includes(city));
      if (!owner) return null;
      covered.add(owner);
    }
    if (covered.size !== new Set(provinces).size) return null;
  }
...
// 只写城市名、不带州的写法（Apple / 诺华 / NXP 的岗位板整批这么写："Beaverton" /
// "Culver City" / "Phoenix, , , E, Camelback"）。州缩写规则对它们无能为力（串里没有州），
// live 实测 298 个地点 / 1,972 个在招岗因此抽不出国家 → 按源 regions 兜底 → 算进国内供给。
// ⚠️ 城市名是**无条件 token**，不受位置/大小写限制，所以只收「没有知名同名外国城市」的：
// Cambridge / Durham / Birmingham / Richmond / Melbourne / Vienna / Dublin / Athens /
// Manchester / Ontario 这类重名的一律不收。与 crawler/geo.py 的 _US_CITY_NAMES 逐条一致。
const US_CITY_NAMES = [
...
// 串尾两字母码撞别国国别码时的护栏（只减不增：命中就不判美国，落回 null）。
// ⚠️ **Ontario 是唯一不能收全称的省**：它既是加拿大省份也是加州的城市。live 实测
//    「含 ontario 且串尾码是 CA」33 个在招岗里 **19 个是加州安大略市**、只有 14 个是加拿大，
//    收它净亏 5 个岗且亏在「把真美国岗判错」这个更糟的方向。其余省份全称净赚 24 岗、误伤 0。
const CA_PROVINCE_CODES = new Set(["AB", "BC", "MB", "NB", "NL", "NS", "NT", "NU", "ON", "PE", "QC", "SK", "YT"]);
// ⚠️ 也**不收裸 "canada"**（会打中加州 La Cañada Flintridge）。
// ⚠️ new brunswick / yukon / quebec 同时是美国地名（新泽西 New Brunswick 是强生总部、
//    俄州 Yukon、丹佛 Quebec St，全库 ~220 岗）——本表**只在串尾码恰好是 CA 时才被查**，
//    那批岗的串尾是 NJ/US/OK/CO，碰不到这里。泛化成「串里有省名就否决」会当场炸掉它们。
...
// ⚠️ 印度邦名不与美国地名重名，码和全称都能收。原规则只挡住小写 "Chennai, TN, in"，
//    大写的 "Mumbai, Maharashtra, IN" 仍会被判成印第安纳。
const IN_REGION_CODES = new Set(("AP AR AS BR CG DL GA GJ HP HR JH JK KA KL MH ML MN MP MZ NL " +
...
function foreignTailCode(text, upperTokens, code) {
  if (code === "CA") {
    for (const t of upperTokens) if (CA_PROVINCE_CODES.has(t)) return true;
    return CA_PROVINCE_NAMES.some((name) => containsToken(text, name));
  }
  if (code === "IN") {
    for (const t of upperTokens) if (IN_REGION_CODES.has(t)) return true;
    return IN_REGION_NAMES.some((name) => containsToken(text, name));
  }
  return false;
}
...
// ⚠️ 必须与 crawler/geo.py 的 ISO_ALPHA2_CODES 逐条一致（tests/geo.test.js 会 deepEqual 对拍）。
// 完整的「为什么开头位置不能复用结尾那张表」与 live 数字写在 crawler/geo.py 的同名段落里，
// 此处不重复。一句话版：**这个位置上美国州缩写比国别码更常见**（`GA, Atlanta…` 117 行 /
// `NY, BROADWAY…` 116 行 / `CA, Burbank…` 50 行），所以码同时是州缩写时一律弃权，
// 唯 CA / IN 在串里另有该国省/邦硬证据时才认；整条规则排在全表最后一步。
// ---------------------------------------------------------------------------
...
// UK 不是 ISO-2（英国是 GB），但外企 ATS 大量这么写（live 30 行）。归一到 GB。
const LEADING_CODE_ALIASES = { UK: "GB" };
// 开头两个大写字母**不是**国别码的实证反例（只收 live 库里真见过的）：
// GM = 通用汽车厂区前缀（实际在密歇根 Warren），不是冈比亚；NA = 卡夫亨氏的「North America」占位，不是纳米比亚；
// LI = LinkedIn 招聘标签「#LI-Remote」（爱德华兹 `LI, REMOTE`，美国远程岗），不是列支敦士登。
const LEADING_CODE_BLOCKLIST = new Set(["GM", "NA", "LI"]);
const TAIL_CODE_GUARDS = new Set(["CA", "IN"]);
// 只认逗号分隔 + 原串大写形态（live 121 个受影响写法里 119 个逗号、0 个空格）。
const LEADING_COUNTRY_RE = /^([A-Z]{2})\s*[,，]/;

/** 开头的两字母大写国别码。只有其它所有规则都认不出来时才会走到这里。 */
function leadingCountryCode(location) {
  if (!location) return null;
  const stripped = String(location).trim();
  const match = LEADING_COUNTRY_RE.exec(stripped);
  if (!match) return null;
  const raw = match[1];
  if (LEADING_CODE_BLOCKLIST.has(raw)) return null;
  const code = LEADING_CODE_ALIASES[raw] || raw;
  if (!ISO_ALPHA2.has(code)) return null;
  if (US_STATE_CODES.has(code)) {
    // 州缩写与国别码撞车：只有串里另有该国省/邦硬证据才认，否则一律弃权。
    if (!TAIL_CODE_GUARDS.has(code)) return null;
    const upper = new Set(
      stripped.split(/[^A-Za-z]+/).filter((t) => t && t === t.toUpperCase() && t !== code)
    );
    if (!foreignTailCode(norm(stripped), upper, code)) return null;
  }
  return code;
}

// ⚠️ 大写 + 串尾仍挡不住两个真撞车的国别码（live 实测都在库里）：
// "Toronto, ON, CA"（加拿大不是加州，42 个在招岗）/ "Mumbai, Maharashtra, IN"（印度不是印第安纳）。
function hasUsState(location) {
  if (!location) return false;
  const stripped = String(location).trim();
  const match = US_STATE_TAIL_RE.exec(stripped);
  if (!match) return false;
  const code = match[1];
  if (!US_STATE_CODES.has(code)) return false;
  if (TAIL_CODE_GUARDS.has(code)) {
    const upper = new Set(
      stripped.split(/[^A-Za-z]+/).filter((t) => t && t === t.toUpperCase() && t !== code)
    );
    if (foreignTailCode(norm(stripped), upper, code)) return false;
  }
  return true;
}
...
COUNTRY_TOKENS.US.push(...US_CITY_NAMES);
...
// 逐字镜像 crawler/geo.py 的同名常量（tests/geo.test.js 会 deepEqual 对拍，改一端必须改另一端）。
const OVERSEAS_LOCATION_TOKENS = new Set([
  "africa", "algeria", "americas", "argentina", "australia", "austria",
  "belgium", "brazil", "britain", "bulgaria", "cambodia", "canada",
  "chile", "colombia", "croatia", "czechia", "denmark", "dubai",
  "ecuador", "egypt", "emea", "estonia", "europe", "finland",
  "france", "germany", "guatemala", "hungary", "india", "indonesia",
  "ireland", "israel", "italy", "japan", "korea", "latam",
  "latvia", "lithuania", "malaysia", "mexico", "morocco", "netherlands",
  "norway", "nz", "pakistan", "philippines", "poland", "portugal",
  "romania", "russia", "serbia", "singapore", "slovakia", "slovenia",
  "spain", "sweden", "switzerland", "thailand", "tunisia", "turkey",
  "uae", "uk", "ukraine", "us", "usa", "vietnam",
  // 2026-09-23 补（理由与刻意不收 "jordan" 的原因见 crawler/geo.py 同名常量）
  "georgia", "england", "scotland", "wales", "greece", "kazakhstan", "bangladesh", "belarus",
  "bermuda"
]);

const OVERSEAS_LOCATION_PHRASES = [
  "aland islands", "costa rica", "dominican republic", "new zealand",
  "puerto rico", "saudi arabia", "south africa", "south korea",
  "sri lanka", "united arab emirates", "united kingdom", "united states"
];

// 与 crawler/geo.py 的 _is_overseas_pinned 同口径：地点里明确钉着一个境外地名
// （"Remote Germany" / "India (Remote)"）。少了这一层，给外企源补 CN 会让它名下
// 所有海外远程岗一起变成国内岗（2026-09-05 实测那批源上有 121 个）。
function isOverseasPinned(location) {
  const text = norm(location);
  if (!text) return false;
  if (OVERSEAS_LOCATION_PHRASES.some((phrase) => text.includes(phrase))) return true;
  return text.split(/[^a-z]+/).filter(Boolean).some((t) => OVERSEAS_LOCATION_TOKENS.has(t));
}
...
function deriveCountryCode(location) {
  ...
  // 所以它只配捡别人都不要的串（`SE, Bothell, Washington, United, States` 早在第一步就判了 US）。
  return leadingCountryCode(location);
}
...
function deriveJobScope(location, regions) {
  ...
  // 地点明确钉着境外地名时，绝不能走下面的 source.regions 兜底（外企源补了 CN 之后
  // 会把它名下的海外远程岗一起染成国内岗）。顺序与 crawler/geo.py 逐字一致。
  if (isOverseasPinned(location)) return "overseas";
...
// ---------------------------------------------------------------------------
// 省级归属（locationProvinces / chinaProvincePlaceNames / CN_PROVINCE_PREFECTURES）：实现与规则说明在
// ./cn-location-provinces.js（2026-09-23 拆出，逻辑未改），这里只再导出，对外 API 不变。
// ---------------------------------------------------------------------------
const { CN_PROVINCE_PREFECTURES, locationProvinces, chinaProvincePlaceNames } = require("./cn-location-provinces");

// 项目口径下**任何 source.regions 都不该放行**的地区（目前只有台湾）。与 crawler/geo.py 的
// is_rejected_location 逐字同口径——两端各自的写入路径（爬虫端 normalizer.validate_job_quality /
// app 端 lib/jobs-store/write.ts）都该在写库前调它，不能只靠 locationInScope 复核，因为部分
// adapter 会绕过或压根不调那道复核（详见 crawler/geo.py 同名函数注释与实测台账）。
const REJECTED_COUNTRY_CODES = new Set(["TW"]);

function isRejectedLocation(location) {
  const code = deriveCountryCode(location);
  return Boolean(code) && REJECTED_COUNTRY_CODES.has(code);
}
