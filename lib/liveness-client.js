...
// （wt req_state=9501 / hotjob state=1017 / workday 403+S22 / detail 404·410），由 tests/liveness-client.test.js golden 钉死。
//
...
// workday：detail = source_url 去尾 /jobs 再拼 jd_url 的 /job/{path}；撤岗 → cxs detail 404/410，
// 或 403 + {"errorCode":"S22"}（Workday 撤岗的主要形态，live 对拍证据见 _detail_workday）。镜像 _detail_workday。
async function livenessWorkday({ jd_url, source_url }) {
  ...
  if (GONE.has(r.status)) return "dead";
  if (r.status === 403) {
    // 双条件：WAF / CDN 的 403 是 HTML，解析失败 → 不判死。
    const j = await r.json().catch(() => null);
    return j && j.errorCode === "S22" ? "dead" : "unknown";
  }
  if (!r.ok) return "unknown";
  const j = (await r.json()) || {};
