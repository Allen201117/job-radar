...
//   2026-09-07 由「项目制投递」改名「公告制招聘」：16 行里 14 行是公告制、项目制 0 行，原名不符事实。
//
// `hint` 是导航图标的悬停说明。图标化导航省了空间，代价是**光看图标猜不出功能**——
// 「公告制招聘」尤其如此（创始人反馈：用户不知道这是干嘛的）。所以每条都配一句人话，
...
export default function NavbarClient({ initialEmail }: { initialEmail: string | null }) {
  const pathname = usePathname();
  const router = useRouter();
  const supabase = createBrowserClient();
  // 当前用户邮箱由服务端外壳 components/Navbar.tsx 透传（它读 middleware 注入的请求头，零网络）。
  // 改造前这里是挂载后 supabase.auth.getUser()——浏览器直连 Supabase 所在区域（悉尼）的一次
  // 跨洋往返，每次换页都跑一遍。现在零网络，且首帧就是正确登录态（不再闪一下未登录的导航栏）。
