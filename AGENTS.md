# 求职雷达 / Job Radar — 项目级 Codex 指南

> 本文件指导 Codex 在本仓库工作。规则优先级：当前明确指令 > 本文件 > 全局 `~/.Codex/AGENTS.md` > 默认行为。
...
⚠️ 在 `.Codex/worktrees/*` 里跑 `next lint` 会因为「主仓 + worktree 两份 .eslintrc.json /
...
2. **⚠️ 父级 AGENTS.md 混淆**：家目录 `~/AGENTS.md` 描述的是另一个项目（余声/YuSheng），会被当作父级上下文加载。本项目是求职雷达，与 YuSheng 无关，冲突时以本文件为准。
