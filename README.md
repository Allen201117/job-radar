...
| events | 自建行为埋点（零分析 SDK）；`/admin/health` 的用户行为漏斗与留存由它派生 | 登录用户只能写自己的，仅 admin 可读 |
...
| 模块 | 做什么 | 入口 |
|---|---|---|
| 今日机会 | 按用户画像分层召回 + 打分，每天给一小撮值得看的岗 | `/today`（`/` 是未登录落地页） |
| 岗位库 | 服务端筛选 + 中文 bigram 全文检索；可按公司刷新 | `/jobs` |
| 校招专区 | 锁定用户行业的目标公司校招岗、届别标签与筛选；配套高频「开闸检测」车道，秋招一开就捞 | `/campus` |
| 洞察库 | 公司维度的事实/经验洞察（三层供给：自有岗位库派生 / 官方源 / 多源检索经验），带分级、时效与去标识门 | `/insights`（一级导航）+ 岗位卡片上的公司抽屉 |
| 公告制招聘 | 央企 / 银行这类只发招聘公告、不挂单个岗位的公司，投递入口单独收拢（2026-09-07 由「项目制投递」改名——实际 16 行里 14 行是公告制、项目制 0 行，原名不符事实） | `/programs` |
| 值得投 / 已投递 | 收藏、忽略、投递进展跟踪，多岗并排对比 | `/saved`、`/applied` |
| 个人主页 | 简历画像与求职目标：文本 / `.txt` / `.md` / PDF / Word / 图片 → 结构化档案 → 用户确认后同步偏好 | `/me`（`/preferences` 保留重定向） |
| 管理员看板 | 五个 tab：总览 / 岗位库 / 必投供给 / **用户行为**（自建埋点算出的主漏斗、留存、活跃分布）/ 系统运行 | `/admin/health`（仅管理员） |
